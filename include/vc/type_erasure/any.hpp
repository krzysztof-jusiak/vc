...
struct accept {
  using type = type_list<
    typename base<TCallable, typename T::call, function_traits_t<decltype(&T::parameters::template operator()<_1>), _1>>::type
  >;
};
...
struct any_impl_base {
  using concepts_t = join_t<typename accept<typename Ts::callable, Ts>::type...>;
  using concept_t = apply_t<concept, concepts_t>;

  template<class T, class... Tx>
  any_impl_base(T t, type_list<Tx...>)
...
struct any : any_impl<typename TConcept::type> {
  template<class T, REQUIRES__(satisfies<T>(TConcept{}))>
  any(T t)
...
template <class... Ts>
struct any_impl<type_list<Ts...>>
  : any_impl_base<Ts...>, base<typename Ts::callable, typename Ts::call, function_traits_t<decltype(&Ts::parameters::template operator()<_1>), any_impl_base<Ts...>>>::type...
{

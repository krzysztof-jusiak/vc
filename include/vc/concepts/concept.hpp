...
struct models<TConcept(Ts...), aux::void_t<
...
struct constraint<std::true_type, T, aux::type_list<T1, T2, Ts...>> {
  using expr = typename base<std::true_type, typename T::call, aux::function_traits_t<decltype(&T::parameters::template operator()<_1>), _1>>::type;
  using constr = decltype(expr::constraint());
  using type = typename models<aux::expr_wrapper<constr>(T1, Ts...)>::type;
};

template<class T, class... Ts>
struct constraint<std::false_type, T, aux::type_list<Ts...>>
  : models<aux::expr_wrapper<typename T::constraint>(Ts...)>::type
{ };

template<class T, class... Ts>
constexpr auto satisfies_impl(aux::type_list<Ts...>) {
  return std::is_same<
    aux::type_list<aux::always<Ts>...>,
    aux::type_list<typename constraint<typename Ts::callable, Ts, aux::function_traits_t<decltype(&Ts::parameters::template operator()<_1>), T>>::type...>
  >::value;
}
...
struct expr_impl {
  ...
  using parameters = T2;
  using constraint = T3;
  using type = aux::type_list<expr_impl>;
